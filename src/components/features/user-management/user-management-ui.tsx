...
import { Pencil, Trash2, Ban, UsersRound, Activity, Calendar } from 'lucide-react'
...
export const userSchema = z.object({
    ...
    status: z.enum(['Active', 'Delete', 'Banned']),
})
...
export interface UserManagementUIProps {
    ...
    onResetSearch: () => void
    onSaveUser: (id: string | null, values: z.infer<typeof userSchema>) => void
            ...
            { key: 'verification', header: 'VERIFICATION', render: (user) => <span className="text-muted-foreground font-medium">{user.verification}</span> },
            {
                ...
                render: (user) =>
                    user.status === 'Active' ? (
                        ...
                        <span className="text-warning font-semibold text-sm">Banned</span>
                    ) : (
                        <span className="text-destructive font-semibold text-sm">Delete</span>
                    ),
            },
            {
                key: 'action',
